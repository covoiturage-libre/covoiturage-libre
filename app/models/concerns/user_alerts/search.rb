  ...
  module Search
    ...
    class_methods do
      ...
      def from_to(from_lon, from_lat, to_lon, to_lat,
        ...
        # Avoid UserAlerts doublon
        matching_points = Point.select("DISTINCT ON (point_a.user_alert_id) point_a.*,
        ...
        joins('INNER JOIN points AS point_b ON point_b.user_alert_id = point_a.user_alert_id').
        where("ST_Dwithin(
               ST_GeographyFromText('SRID=4326;POINT(' || point_a.lon || ' ' || point_a.lat || ')'),
        ...
        order('point_a.user_alert_id ASC, point_ab_distance ASC')

        select('user_alerts.*,
          point_a_id, point_a_price,
          point_b_id, point_b_price,
          user_alerts.id'). # user_alerts.id is necessary here for the COUNT_COLUMN method used by Kaminari counting.
        joins("INNER JOIN (#{matching_points.to_sql}) AS point_a ON user_alerts.id = point_a.user_alert_id")
      end
      ...
      def from_only(from_lon, from_lat, from_around_distance = DEFAULT_SEARCH_AROUND_DISTANCE_IN_METERS)
        ...
        .order('point_a.user_alert_id ASC, point_a_distance ASC')
      ...
      def to_only(to_lon, to_lat, to_around_distance = DEFAULT_SEARCH_AROUND_DISTANCE_IN_METERS)
        ...
        # Avoid UserAlerts doublon
        matching_points = Point.select("DISTINCT ON (point_b.user_alert_id) point_b.*,
          point_b.id AS point_b_id, point_b.price AS point_b_price,
          ST_Distance(
            ST_GeographyFromText('SRID=4326;POINT(' || point_b.lon || ' ' || point_b.lat || ')'),
            ST_GeographyFromText('SRID=4326;POINT(#{to_lon.to_f} #{to_lat.to_f})')
        ...
        .order('point_b.user_alert_id ASC, point_b_distance ASC')

        select('user_alerts.*,
          point_b_id, point_b_price, point_b_distance,
          user_alerts.id'). # user_alerts.id is necessary here for the COUNT_COLUMN method used by Kaminari counting.
        joins("INNER JOIN (#{matching_points.to_sql}) AS point_b ON user_alerts.id = point_b.user_alert_id")
      end
